...
class StateEstimator:
    ...
    def get_body_angular_vel(self):
        # защитимся от нулевых dt и NaN в первых сэмплах
        safe_dt = np.where(self.dt_history == 0, 1e-6, self.dt_history)
        deuler_over_dt = self.deuler_history / safe_dt
        deuler_over_dt = np.nan_to_num(deuler_over_dt, nan=0.0, posinf=0.0, neginf=0.0)
        self.body_ang_vel = self.smoothing_ratio * np.mean(deuler_over_dt, axis=0) + (
